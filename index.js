    ...
    */
            ...
            functions.signIn({
                ...
                .catch(err => {
                    if(req.xhr && err.status !== undefined && err.status>=400){
                        return res.json({error:{status:err.status, message: err.message}});
                    }
    ...
    if (functions.sendSignInEmail) {
        ...
        expressApp.post(`${pathPrefix}/email/signin`, (req, res) => {
            ...
            functions.find({email: email}, req)
                .then(user => {
                    if (user) {
                        // If a user with that email address exists already, update token.
                        user.emailToken = token
                        return functions.update(user, null, null, req)
                    } else {
        ...
        expressApp.get(`${pathPrefix}/email/signin/:token`, (req, res) => {
            ...
            functions.find({emailToken: req.params.token}, req)
                .then(user => {
                    if (user) {
                        ...
                        return functions.update(user, null, {delete: 'emailToken'}, req)
                    } else {
